<?php
// Không định nghĩa lại các hằng số nếu đã có
if (!defined('BASE_URL')) {
...
// Kết nối CSDL - bao gồm config trước session_start
...
// Reset số lần redirect để tránh vòng lặp
...
// Khởi tạo biến thông báo lỗi/success
$error = "";
$success = "";
$category_error = "";
$category_success = "";
...
$notifQuery = "SELECT COUNT(*) as unread_count FROM notifications WHERE user_id = ? AND is_read = 0";
$notifStmt = $conn->prepare($notifQuery);
$notifStmt->execute([$user_id]);
$notifData = $notifStmt->fetch();
$unreadNotifs = $notifData['unread_count'] ?? 0;

// Xử lý thêm danh mục mới (form modal)
if (isset($_POST['add_category'])) {
    $category_name = trim($_POST['category_name']);
    $category_description = trim($_POST['category_description'] ?? '');
    
    if (!empty($category_name)) {
        // Kiểm tra xem danh mục đã tồn tại chưa - using PDO
        $checkQuery = "SELECT category_id FROM categories WHERE name = ?";
        $checkStmt = $conn->prepare($checkQuery);
        $checkStmt->execute([$category_name]);
        $result = $checkStmt->fetchAll();
        
        if (count($result) == 0) {
            // Thêm danh mục mới - using PDO
            $catQuery = "INSERT INTO categories (name, description, status) VALUES (?, ?, 1)";
            $catStmt = $conn->prepare($catQuery);
            
            if ($catStmt->execute([$category_name, $category_description])) {
                $category_success = "Danh mục '$category_name' đã được thêm thành công!";
                $new_category_id = $conn->lastInsertId();
            } else {
                $category_error = "Không thể thêm danh mục: " . $catStmt->errorInfo()[2];
            }
        } else {
...
// Xử lý tạo khóa học mới (form chính)
if ($_SERVER['REQUEST_METHOD'] == 'POST' && !isset($_POST['add_category'])) {
    $title = trim($_POST['title']);
    $description = trim($_POST['description']);
    $category_id = !empty($_POST['category_id']) ? intval($_POST['category_id']) : NULL;
    if (is_null($category_id)) {
        $error = "Vui lòng chọn danh mục cho khóa học.";
    }
    ...
    if (isset($_FILES['image']) && $_FILES['image']['error'] == UPLOAD_ERR_OK) {
        ...
        $error = "Vui lòng chọn hình ảnh cho khóa học.";
    }

    if (empty($error)) {
        // Chuẩn bị truy vấn thêm khóa học mới - using PDO
        $courseQuery = "INSERT INTO courses (instructor_id, category_id, title, description, price, image, level, language, requirements, learning_outcomes, tags) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
        $courseStmt = $conn->prepare($courseQuery);
        $params = [$user_id, $category_id, $title, $description, $price, $image, $level, $language, $requirements, $learning_outcomes, $tags];

        if ($courseStmt->execute($params)) {
            $success = "Khóa học đã được tạo thành công!";
        } else {
            $error = "Có lỗi xảy ra khi tạo khóa học: " . $courseStmt->errorInfo()[2];
        }
    }
}

// Lấy danh sách danh mục từ database - using PDO
$categoryQuery = "SELECT category_id, name FROM categories WHERE status = 1 ORDER BY name ASC";
...
<body>
    ...
    <div class="sidebar" id="sidebar">
        <ul>
            ...
            <li><a href="<?php echo BASE_URL; ?>app/views/product/create_course.php" class="active"><i class="fas fa-plus-circle"></i> Thêm Khóa Học</a></li>
